...
# __file__ = 이 config.py의 경로 → parent.parent = backend/
_BACKEND_DIR = Path(__file__).resolve().parent.parent
# backend의 부모 = 레포 루트
_REPO_ROOT = _BACKEND_DIR.parent

# 레포 루트 → backend 순으로 로드 (뒤에 나온 값이 우선되지 않게 루트 먼저)
# load_dotenv는 이미 설정된 환경변수를 기본적으로 덮어쓰지 않는다.
...
# DATABASE_URL 환경변수가 있으면 그걸 쓰고, 없으면 기본 SQLite 경로를 쓴다.
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    f"sqlite:///{_DEFAULT_SQLITE_PATH}",
)

# OpenRouter API 키. 없으면 빈 문자열 (호출 시점에 오류로 처리).
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "")
# 사용할 모델 id. 환경변수로 바꿀 수 있다.
